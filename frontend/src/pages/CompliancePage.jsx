...
import { CertificateMock, CERT_ITEMS } from '../components/PortfolioMocks';
...
export default function CompliancePage() {
  return (
    <>
      ...
      <section className="relative py-24 md:py-36 border-t border-white/5 overflow-hidden" data-testid="compliance-pillars">
        ...
        <div className="relative max-w-7xl mx-auto px-6 lg:px-12">
          ...
          <div className="grid md:grid-cols-2 gap-5">
            {pillars.map((p, i) => (
              <FadeUp key={p.num} delay={(i % 2) * 0.1}>
                <div
                  className="group relative rounded-2xl border border-white/10 bg-white/[0.02] p-8 h-full hover:border-[#EF2B3B]/60 transition-all overflow-hidden"
                  data-testid={`compliance-pillar-${p.num}`}
                >
                  <div className="absolute -top-16 -right-16 h-40 w-40 rounded-full bg-[#EF2B3B]/5 blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
                  <div className="relative flex items-start justify-between mb-8">
                    <div className="h-12 w-12 rounded-full bg-[#EF2B3B]/10 flex items-center justify-center text-[#EF2B3B] group-hover:bg-[#EF2B3B] group-hover:text-black transition-colors">
                      <p.icon size={20} />
                    </div>
                  ...
                  <ul className="relative space-y-3">
                    {p.items.map((it) => (
                      <li key={it} className="flex items-start gap-3 text-zinc-300">
                        <Check size={14} className="mt-1.5 text-[#EF2B3B]/80 flex-shrink-0" />
                        <span>{it}</span>
                      </li>
                    ))}
                  </ul>
          ...
          <div className="mt-28">
            <FadeUp>
              <Slideshow
                items={CERT_ITEMS}
                renderItem={(c) => <CertificateMock cert={c} />}
                eyebrow="Paperwork · Documents we handle"
                title="From CIPC to SARS to B-BBEE — all handled."
                testId="compliance-slideshow-certs"
              />
            </FadeUp>
          </div>
        </div>
      ...
      <section className="relative py-28 md:py-40 border-t border-white/5 overflow-hidden" data-testid="compliance-why">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-[600px] w-[600px] rounded-full bg-[#EF2B3B]/10 blur-[180px] pointer-events-none" />
        <div className="relative max-w-4xl mx-auto px-6 lg:px-12 text-center">
          <FadeUp>
            <div className="inline-flex items-center gap-3 font-mono text-[11px] tracking-[0.3em] uppercase text-[#EF2B3B] mb-8">
              <Scale size={14} />
